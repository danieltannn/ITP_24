...
<details><summary> The codes in this section will go into the /usr/bin/compositeusb file in the Raspberry Pi under the header "Gadget functions will be added here". Click to expand </summary>
   ...
   ```
   This will turn the Raspberry Pi Zero W into a HID keyboard. However, it would still require additional scripts to send keystrokes to the HOST PC (either automatically or manually.)
   ...
   ```
   Enable login via gadget serial port on the console
   ```
