...
"@
            while(1){
                ...
                if (Is_Connected -ne $null){
                    ...
                    $result = Invoke-WebRequest -Uri http://daniel.local/ -Method POST -Body ($data|ConvertTo-Json) -ContentType "application/json"
                    $result.content >> C:/Users/Daniel/Desktop/test.txt
                    Start-Sleep -s 5
                }
                else{break}
            }
