...
"@
            while(1){
                ...
                if (Is_Connected -ne $null){
                    ...
                    Invoke-WebRequest -Uri $sending_base -UseBasicParsing -Method POST -Body ($json_data|ConvertTo-Json) -ContentType "application/json"

                    #Retrieving interval
                    $tag = Encode("AWD")
                    $url = ($interval_base + "uuid=" + $studentId + "&category=" + $tag)
